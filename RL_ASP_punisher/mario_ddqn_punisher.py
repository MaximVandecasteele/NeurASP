...
import pickle
from symbolic_components.Punisher import Punisher
...
parser.add_argument("-wd","--working_dir",help='Where will files be stored to and loaded from',type=str,default="Test") #required=True
...
def run(asp, training_mode, pretrained):
    ...
    config = {
            'game_rules': '/Users/maximvandecasteele/PycharmProjects/NeurASP/RL/asp/game_rules.lp',
            'show_constraints': '/Users/maximvandecasteele/PycharmProjects/NeurASP/RL/asp/show_constraints.lp'
        }

    punisher = Punisher(config)
   
    env = gym_super_mario_bros.make('SuperMarioBros-'+level+'-v0') #Load level


    env = make_asp_punisher_env(env) # Wraps the environment so that frames are 15x16 ASP frames
 # Wraps the environment so that frames are grayscale / segmented
    ...
    for ep_num in tqdm(range(epochs)):
        ...
        while True:
            ...
            if not terminal:
                ...
                reward = punisher.punish(facts, reward, action[0])
            else:
                reward = -15.0
